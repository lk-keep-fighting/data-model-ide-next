...
| Prisma 模型 | 关键字段 | 说明 |
| ---- | ---- | ---- |
| `DataStorageModel` | `id`, `name`, `database`, `schema`, `tables`, `operations`, `views` | 存储数据库连接信息与同步得到的表结构快照。`schema.tables` 保存字段元数据，供视图/表单构建参考。 |
| `DataStorageTable` | `id`, `name`, `schema`, `storageModelId`, `views`, `forms` | 对应单个数据表；`schema.columns` 描述字段类型、主键、注释等信息。 |
| `DataViewModel` | `id`, `name`, `description`, `layout`, `storageModelId`, `storageTableId` | 列表视图模型，`layout.fields` 定义列清单及排序能力。 |
| `DataFormModel` | `id`, `name`, `schema`, `storageTableId`, `operations` | 表单模型，`schema.fields` 描述组件类型、是否必填等；可通过 `schema.meta.usage` 区分筛选与提交。 |
| `DataOperationModel` | `id`, `name`, `type`, `endpoint`, `method`, `storageModelId`, `formModelId`, `requestSchema`, `responseSchema` | 操作模型，描述 API 行为及表单绑定关系。`requestSchema`/`responseSchema` 以 JSON 保存接口契约与 UI 元信息。 |

### 16.2 AdminTable 模型映射关系

结合第 4 章的模型结构与上表的存储实体，生成器应遵循以下映射规则：

- **视图 (`models.view`)**
  - 从 `DataViewModel` 读取，`layout.fields` 按顺序转换为 `TableColumnConfig`；`column` → `dataIndex`，`label` → `label`，`type` 可用于推导 `renderType`。
  - 若定位符的 `operationIds` 中存在 `type = "READ"` 的操作，可将其转换为 `DataSourceConfig`（`endpoint`、`method`、`requestSchema.body` → `bodyTemplate`、`responseSchema.meta?.mapping` → `responseMapping`）。
  - 当 `layout.pagination` 或 `schema.meta.pagination` 存在时，可填充 `view.pagination`；否则使用系统默认值。

- **筛选表单 (`models.filterForms`)**
  - 选取定位符中的 `filterFormIds`，加载对应的 `DataFormModel`。
  - `schema.meta.usage` 需为 `"filter"`；`schema.fields` 转换为 `FilterConfig`。
    - 推荐在表单字段中扩展 `component` 与筛选类型的对应关系：`text/textarea → "text"`、`number → "number"`、`select → "select"`、`date → "date-range"`。
    - 选项类字段可通过 `schema.fields[].options` 持久化，以映射到 `FilterConfig.options`。

- **提交表单 (`models.submissionForms`)**
  - 选取定位符中的 `submissionFormIds`（缺省时默认取同表的全部 `usage !== "filter"` 表单）。
  - 将 `schema.fields` 映射成 `ActionFormConfig` 字段：`column` → `id`，`label` → `label`，`component` → `type`，并迁移 `required`、`defaultValue`、`options` 等额外属性。

- **操作 (`models.operations`)**
  - 依据定位符的 `operationIds` 加载 `DataOperationModel`。
  - `endpoint`、`method` 映射为 `ActionBehavior`，其中 `requestSchema.body`、`requestSchema.query` 可转为 `bodyTemplate`/`query`。
  - 建议在 `requestSchema.ui` 中持久化 UI 元信息，用于补齐 `scope`、`intent`、`icon`、`confirm`、`requiresSelection` 等字段；未声明时按默认值处理。
  - 仍然可以通过 `formModelId` 生成 `formRef`，或在 `requestSchema.inlineForm` 中内联表单定义以映射到 `operation.form`。
  - `responseSchema` 可保存接口返回结构，供调试或后续文档生成使用。

- **兼容字段合并**
  - 若 `AdminTablePageConfig` 仍包含传统字段（如 `table.columns`、`headerActions`），生成器在归一化阶段可作为兜底，但模型驱动数据优先级更高。

### 16.3 模型定位与生成流程示例

1. 读取 `AdminTableModelLocator`，并校验 `storageModelId`、`storageTableId` 的关系是否成立。
2. 拉取对应 `DataStorageModel`，并定位到指定的 `DataStorageTable`、`DataViewModel`。
3. 根据 `filterFormIds`/`submissionFormIds` 加载 `DataFormModel`，并按上述规则映射成筛选与提交表单。
4. 加载 `operationIds` 对应的 `DataOperationModel`，补齐行为、表单引用与 UI 元信息。
5. 组合得到完整的 `AdminTablePageConfig`，供运行时或设计器预览使用。
6. 若任一模型缺失或数据不一致，应在生成阶段返回可读的错误信息，提示设计器补齐数据。

示例定位符：

```json
{
  "storageModelId": "sm_users",
  "storageTableId": "tbl_users",
  "viewId": "view_users_default",
  "filterFormIds": ["form_users_filter"],
  "submissionFormIds": ["form_users_edit"],
  "operationIds": ["op_fetch_users", "op_create_user", "op_disable_user"]
}
```

基于上述定位符，生成器可输出如下结构（示例，省略部分字段）：
...
```json
{
  ...
  "models": {
    "view": {
      ...
      "columns": [
        { "id": "username", "label": "用户名", "dataIndex": "username", "sortable": true },
        { "id": "email", "label": "邮箱", "dataIndex": "email" }
      ]
    },
    "filterForms": [
      {
        "id": "form_users_filter",
        "filters": [
          { "id": "keyword", "label": "关键词", "type": "text", "field": "username" },
          { "id": "status", "label": "状态", "type": "select", "options": ["enabled", "disabled"] }
        ]
      }
    ],
    "submissionForms": [
...
该规范确保存储层、设计器与前端渲染层之间的语义一致，便于后续扩展模型管理、自动化生成及预览能力。
